...
# ============================================================================
# DASHBOARD ENDPOINT — returns all data for the student/teacher main page
# ============================================================================

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def student_dashboard(request, course_id):
    """Return modules, submissions, accessibility, question counts, and announcements in one call."""
    user = request.user
    course = get_object_or_404(Course, pk=course_id)

    # Verify enrollment
    if user.isStudent:
        if not CourseToStudents.objects.filter(course=course, user=user).exists():
    ...
    student_grades_lookup = {}
    for g in UserQuestionGrade.objects.filter(question__module__in=modules):
        student_grades_lookup[(g.user_id, g.question_id)] = g

    # Build lookup dicts
    questions_by_module = {}
    for q in all_questions:
        questions_by_module.setdefault(q.module_id, []).append(q)
    ...
    for module in modules:
        ...
        mod_submissions = submissions_by_module.get(module.id, [])
        user_submissions = [s for s in mod_submissions if s.user_id == user.id]

        # Accessibility check for students
        if user.isStudent:
            is_accessible = module.is_posted
            if is_accessible:
                # Check all previous posted modules are completed
                for prev in modules:
                    if prev.module_order >= module.module_order:
                        break
                    if not prev.is_posted:
                        continue
                    prev_questions = questions_by_module.get(prev.id, [])
                    if not prev_questions:
                        is_accessible = False
                        break
                    prev_user_subs = [s for s in submissions_by_module.get(prev.id, []) if s.user_id == user.id]
                    submitted_question_ids = {s.question_id for s in prev_user_subs}
                    if not all(q.id in submitted_question_ids for q in prev_questions):
                        is_accessible = False
                        break

            is_completed = False
            if mod_questions:
                submitted_ids = {s.question_id for s in user_submissions}
                is_completed = all(q.id in submitted_ids for q in mod_questions)
        else:
            is_accessible = True
            is_completed = False

        # Serialize user submissions
        ...
        module_data.append({
            ...
            "custom_module_name": getattr(module, 'custom_module_name', None),
            "module_description": module.module_description,
            "module_order": module.module_order,
            "is_posted": module.is_posted,
            "youtube_link": module.youtube_link,
            "due_date": module.due_date.isoformat() if module.due_date else None,
            "question_count": len(mod_questions),
    ...
    for a in announcements:
        announcement_data.append({
            ...
            "created_by_name": f"{a.created_by.first_name} {a.created_by.last_name}" if a.created_by else "",
        })
...
def teacher_dashboard(request, course_id):
    ...
    all_grades = {}
    for g in UserQuestionGrade.objects.filter(question__module__in=modules):
        all_grades[(g.user_id, g.question_id)] = g
    ...
    for student in students:
        ...
        for module in posted_modules:
            ...
            if module.due_date:
                ...
                due = module.due_date.date() if hasattr(module.due_date, 'date') else module.due_date
